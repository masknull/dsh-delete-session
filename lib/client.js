...
//      carrying `node.id` plus `onRename`) is eligible for injection. The
//      session id and title are resolved from those same props at injection
//      time: there is no cached "last clicked row" state that could go stale
//      and act on the wrong session.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * object carrying `node` (with a string id) plus an `onRename` function is
     * unambiguous in this tree. Returns null for menus that are not a session
     * row's menu.
